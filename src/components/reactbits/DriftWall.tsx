...
// Source: React Bits — "Drift Wall" (TS + CSS variant, npx shadcn add
// @react-bits/DriftWall-TS-CSS). The algorithm/architecture is kept faithful
// to the original; the only addition is the `interactive` prop (default true =
// original behaviour) so the wall can also be used as a purely decorative,
// non-focusable, click-through background.
...
export interface DriftWallProps {
  ...
  overlayColor?: string;
  /**
   * When false the tiles are inert — not focusable, no per-tile hover lift —
   * and the wall never intercepts pointer or keyboard input (parallax still
   * follows the cursor via a window listener). Use for decorative background
   * placement. Defaults to true, the faithful React Bits behaviour.
   */
  interactive?: boolean;
  className?: string;
  style?: CSSProperties;
...
const DriftWall = ({
  ...
  // Decorative mode: parallax still follows the cursor, but from a window
  // listener so the wall itself never needs to receive pointer events.
  useEffect(() => {
    if (interactive || parallax <= 0 || reduced) return;
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    if (!interactive) {
      return (
        <div key={id} className="drift-wall__tile" data-col={colIndex} aria-hidden="true">
          {inner}
        </div>
      );
    }
  ...
  return (
    <div
      ...
      onPointerEnter={() => {
        if (interactive) wallHoveredRef.current = true;
      }}
